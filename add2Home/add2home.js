...
/*		if ( !isReturningVisitor ) w.localStorage.setItem('addToHome', Date.now());
		else if (options.expire == -1 && isExpired) w.localStorage.setItem('addToHome', Date.now() + 129600 * 60000);//Sets expiration for 3 months
		else if ( options.expire && isExpired ) w.localStorage.setItem('addToHome', Date.now() + options.expire * 60000);*/
		if ( !isReturningVisitor ) setCookie('addToHome', Date.now());
		else if (options.expire == -1 && isExpired) setCookie('addToHome', Date.now() + 129600 * 60000);//Sets expiration for 3 months
		else if ( options.expire && isExpired ) setCookie('addToHome', Date.now() + options.expire * 60000);
//		console.log(overrideChecks+","+isSafari+","+isExpired+","+isSessionActive+","+isStandalone+","+isReturningVisitor);
	...
	// MUST BE MODIFIED TO WORK WITH COOKIES
	function reset () {
...
function getCookie(cookie){
	if($.cookie(cookie) === "undefined" || $.cookie(cookie) === null){//no cookie
		return false;
	}else{
...
function setCookie(name,value){
	$.cookie(name , value,{expires: 180,  path: "/"});
}
